...
$cssFiles = \Arr::get($themeFiles, 'default.css');
$session = \Session::user();

if (!empty($session->user__theme) && $session->user__theme !== 'default') {
    $theme = $session->user__theme;
    $cssMerge = \Arr::get($themeFiles, $theme.'.css');
    $cssFiles = \Arr::merge($cssFiles, $cssMerge);
}
if ($assets_minified) {
        foreach ($cssFiles['minified'] as $cssFile) {
            ?>
            <link rel="stylesheet" href="<?=$cssFile?>">
        <?php
        }
} else {
    foreach ($cssFiles['raw'] as $cssFile) {
        ?>
        <link rel="stylesheet" href="<?=$cssFile?>">
    <?php
    }
